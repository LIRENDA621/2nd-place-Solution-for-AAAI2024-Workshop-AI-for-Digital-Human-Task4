...
def infer(data_root, g_body, g_face, g_body2, exp_name, infer_loader, infer_set, device, norm_stats, smplx,
    ...
    for bat in infer_loader:
        ...
        # if poses_.shape[-1] == 300: # 只推理十秒的test音频
        if poses_.shape[-1] >= 0: # 只推理十秒的test音频
            j = j + 1
            if j > 1000:
                continue
            ...
            betas = bat['betas'][0].to(torch.float64).to('cuda') # (1, 300)
            ...
            vertices_list, _ = get_vertices(smplx_model, betas, result_list, config.Data.pose.expression) # (300, 10475, 3) gt&pred
            ...
            # rendertool._render_sequences(cur_wav_file, vertices_list[:1], stand=stand, face=face)
            rendertool._render_sequences(cur_wav_file, vertices_list[:], stand=stand, face=face)
...
def main():
    ...
    smplx_path = '/data/lirenda621/code/TalkSHOW/visualise'
    ...
    rendertool = RenderTool('/data/lirenda621/code/TalkSHOW/visualise/video/' + config.Log.name)
