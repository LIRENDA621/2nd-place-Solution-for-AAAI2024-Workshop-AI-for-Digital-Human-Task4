...
class RenderTool():
    def __init__(self, infer_cfg):
        ...
        path = '/nws/user/lirenda621/Code/TalkSHOW/visualise/smplx/SMPLX_NEUTRAL.npz'
        model_data = np.load(path, allow_pickle=True)
        data_struct = Struct(**model_data)
        self.f = data_struct.f # ?
        self.infer_cfg = infer_cfg
        self.out_path = os.path.join(infer_cfg.save_root_path, infer_cfg.file_name,'video')
    ...
    def _render_sequences(self, cur_wav_file, v_list, j=-1, stand=False, face=False, whole_body=False, run_in_parallel=False, transcript=None):
        ...
        if run_in_parallel:
            ...
            # directory = os.path.join(self.out_path, cur_wav_file.split(symbol)[-5], cur_wav_file.split(symbol)[-4])
            ...
            if j == -1:
                ...
                # video_fname = os.path.join(directory, '%s.mp4' % cur_wav_file.split(symbol)[-1].split('.')[-2].split(symbol)[-1])
                video_fname = os.path.join(directory, self.infer_cfg.speaker_name + '_' + self.infer_cfg.file_name + '.mp4')
            ...
            self._render_sequences_helper(video_fname, cur_wav_file, v_list, stand, face, whole_body, transcript, self.infer_cfg.fps)

    def _render_sequences_helper(self, video_fname, cur_wav_file, v_list, stand, face, whole_body, transcript, fps=30):
        ...
        if whole_body:
            ...
            # width, height = 900, 2700
            # viewport_height = 2700
            # z_offset = 4
        ...
        if int(cv2.__version__[0]) < 3:
            print('cv2 < 3')
            writer = cv2.VideoWriter(tmp_video_file.name, cv2.cv.CV_FOURCC(*'mp4v'), fps, (width, height), True)
        else:
            print('cv2 >= 3')
            writer = cv2.VideoWriter(tmp_video_file.name, cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height), True)

        center = np.mean(v_list[0][0], axis=0)

        # r = pyrender.OffscreenRenderer(viewport_width=900, viewport_height=viewport_height)
        ...
        for i_frame in tqdm(range(num_frames)):
            ...
            for i in range(len(v_list)):
                if face:
                    ...
                    img = render_mesh_helper((v_list[i][i_frame], self.f), center, camera='o', r=r, y=0.7, z_offset=z_offset, infer_cfg=self.infer_cfg, idx=i_frame)
