...
from .wav2vec import MyWav2Vec2Model
...
class Generator(nn.Module):
    def __init__(self,
        ...
        if self.encoder_choice == 'meshtalk':
            ...
            self.audio_encoder = MyWav2Vec2Model.from_pretrained("facebook/wav2vec2-base-960h")  # "vitouphy/wav2vec2-xls-r-300m-phoneme""facebook/wav2vec2-base-960h"
    ...
    def forward(self, in_spec, gt_poses=None, id=None, pre_state=None, time_steps=None): # 最后输出jaw + face = 3 + 100
        if self.training:
            time_steps = gt_poses.shape[1] # 帧数
        ...
        if self.encoder_choice == 'meshtalk':
            ...
            hidden_states = self.audio_encoder(in_spec.reshape(in_spec.shape[0], -1), frame_num=time_steps).last_hidden_state # torch.Size([1, 300, 768])
            feature = self.audio_feature_map(hidden_states).transpose(1, 2) # torch.Size([1, 256, 300])
        else:
            feature, hidden_state = self.audio_encoder(in_spec, pre_state, time_steps=time_steps)

        # hidden_states = in_spec

        feature, _ = self.audio_middle(feature, id=id) # torch.Size([1, 256, 300])
        ...
        for i in range(self.decoder.__len__()):
            ...
            out.append(mid) # torch.Size([1, 3, 300])->jaw torch.Size([1, 100, 300])->face 

        out = torch.cat(out, dim=1)
        out = out.transpose(1, 2)

        return out, None # torch.Size([1, 300, 103])
