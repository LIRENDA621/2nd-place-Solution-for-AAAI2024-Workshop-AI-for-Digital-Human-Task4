...
class AudioEncoder(nn.Module):
    ...
    def forward(self, x, frame_num=0):
        h = self.project(x) # (128, 64, 88)
        h = self._enc_1(h)
        h = self._down_1(h) # (128, 128, 44)
        h = self._enc_2(h)
        h = self._down_2(h)
        h = self._enc_3(h) # (128, 256, 22)
        return h
...
class Encoder(nn.Module):
    def __init__(self, in_dim, embedding_dim, num_hiddens, num_residual_layers, num_residual_hiddens):
        ...
        self.project = ConvNormRelu(in_dim, self._num_hiddens // 4, leaky=True) # conv norm relu

        self._enc_1 = Res_CNR_Stack(self._num_hiddens // 4, self._num_residual_layers, leaky=True) # self._num_residual_layers层conv_norm_relu + redisual
        self._down_1 = ConvNormRelu(self._num_hiddens // 4, self._num_hiddens // 2, leaky=True, residual=True,
                                    sample='down')
        self._enc_2 = Res_CNR_Stack(self._num_hiddens // 2, self._num_residual_layers, leaky=True)
        self._down_2 = ConvNormRelu(self._num_hiddens // 2, self._num_hiddens, leaky=True, residual=True, sample='down')
    ...
    def forward(self, x):
        h = self.project(x) # (128, 39, 88) (128, 256, 88)
        h = self._enc_1(h) 
        h = self._down_1(h) # (128, 512, 44)
        h = self._enc_2(h) 
        h = self._down_2(h) # (128, 1024, 22)
        h = self._enc_3(h)
        h = self.pre_vq_conv(h) # (128, 64, 22)
        return h
...
class Decoder(nn.Module):
    ...
    def forward(self, h, last_frame=None):
        ...
        h = self.aft_vq_conv(h) # torch.Size([128, 64, 22]) torch.Size([128, 1024, 22])
        h = self._dec_1(h) # torch.Size([128, 1024, 22])
        h = self._up_2(h) # torch.Size([128, 512, 44])
        h = self._dec_2(h)
        h = self._up_3(h) # torch.Size([128, 256, 88])
        h = self._dec_3(h)

        recon = self.project(h) # # torch.Size([128, 90, 88])
        return recon, None
...
class VQVAE(nn.Module):
    ...
    def forward(self, gt_poses, id=None, pre_state=None):
        ...
        e, e_q_loss = self.vq_layer(z) # z (bs, 64, 22=88 // 4)
    ...
    def encode(self, gt_poses, id=None):
        z = self.encoder(gt_poses.transpose(1, 2)) # torch.Size([128, 64, 22])
        e, latents = self.vq_layer(z) # e(128, 64, 22)量化后的向量， latent（128，22）对应codebook的索引
        return e, latents
    ...
    def decode(self, b, w, e=None, latents=None, pre_state=None):
        if e is not None:
            ...
            e = self.vq_layer.quantize(latents) # (1, 75, 64)
            e = e.view(b, w, -1).permute(0, 2, 1).contiguous()
            x = self.decoder(e, pre_state.transpose(1, 2) if pre_state is not None else None) # torch.Size([1, 39, 300])
        return x
