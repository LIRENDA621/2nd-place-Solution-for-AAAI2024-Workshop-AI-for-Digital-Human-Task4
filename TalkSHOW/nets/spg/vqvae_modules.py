...
class ExponentialMovingAverage(nn.Module):
    ...
    def forward(self, value):
        self.counter += 1 # value=每个索引在当前bs*seq中出现的次数
        self.hidden.sub_((self.hidden - value) * (1 - self.decay)) # (self.hidden - value) * (1 - self.decay)新值的修正乘权重
...
class VectorQuantizerEMA(nn.Module):
    ...
    def __init__(self, embedding_dim, num_embeddings, commitment_cost, decay,
        ...
        embeddings = torch.empty(self.num_embeddings, self.embedding_dim) # (2048, 64)
    ...
    def forward(self, x):
        ...
        # [B, H, W, C] -> [BHW, C] [128*22, 64]
        flat_x = x.reshape(-1, self.embedding_dim) 

        encoding_indices = self.get_code_indices(flat_x) # BHW=128*22,
        quantized = self.quantize(encoding_indices) # (BHW, 64)
        quantized = quantized.view_as(x)  # [B, W, C]

        if not self.training:
            quantized = quantized.permute(0, 2, 1).contiguous() # (128, 64, 22)
            return quantized, encoding_indices.view(quantized.shape[0], quantized.shape[2]) # (128, 22)

        # update embeddings with EMA
        with torch.no_grad():
            encodings = F.one_hot(encoding_indices, self.num_embeddings).float() # (BHW, code_num)
        ...
        loss = self.commitment_cost * e_latent_loss # self.commitment_cost相当于论文中loss计算的beta
