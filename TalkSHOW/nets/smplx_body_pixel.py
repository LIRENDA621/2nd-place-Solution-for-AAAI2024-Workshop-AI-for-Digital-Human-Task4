...
class TrainWrapper(TrainWrapperBaseClass):
    ...
    def __init__(self, args, config):
        ...
        self.epoch = 0
        self.init_params()
        self.num_classes = self.config.Model.body_num_classes
        self.audio = True
    ...
    def __call__(self, bat):
        ...
        poses = poses[:, self.c_index, :] # torch.Size([128, 129, 88])

        aud = aud.permute(0, 2, 1) # torch.Size([128, 88, 64])
        gt_poses = poses.permute(0, 2, 1) # torch.Size([128, 88, 129])
        ...
        with torch.no_grad():
            ...
            if torch.cuda.device_count() > 1:
                ...
                _, body_latents = self.g_body.encode(gt_poses=gt_poses[..., :self.each_dim[1]], id=id) # (128,22)是GT对应codebook的索引，取值范围0-2047
                _, hand_latents = self.g_hand.encode(gt_poses=gt_poses[..., self.each_dim[1]:], id=id)
            latents = torch.cat([body_latents.unsqueeze(dim=-1), hand_latents.unsqueeze(dim=-1)], dim=-1) # （128, 22, 2）
            latents = latents.detach()

        if self.audio:
            audio = self.audioencoder(aud[:, :].transpose(1, 2), frame_num=latents.shape[1]*4).unsqueeze(dim=-1).repeat(1, 1, 1, 2) # (128, 256, 22, 2)
            logits = self.generator(latents[:, :], id, audio) # torch.Size([128, 2048, 22, 2])
        else:
            logits = self.generator(latents, id)
        logits = logits.permute(0, 2, 3, 1).contiguous() # torch.Size([128, 22, 2, 2048])

        self.generator_optimizer.zero_grad()
        if self.audio:
            self.audioencoder_optimizer.zero_grad()

        loss = F.cross_entropy(logits.view(-1, logits.shape[-1]), latents.view(-1)) # (bs*22*2, 2048) (bs*22*2)
        loss.backward()
    ...
    def infer_on_audio(self, aud_fn, initial_pose=None, norm_stats=None, exp=None, var=None, w_pre=False, rand=None,
        ...
        if continuity:
            ...
            aud_feat = get_mfcc_ta(aud_fn, sr=sr, fps=fps, smlpx=True, type='mfcc', am=am) # (300 64)
        aud_feat = aud_feat.transpose(1, 0)
        aud_feat = aud_feat[np.newaxis, ...].repeat(B, axis=0) # (1, 64, 300)
        ...
        with torch.no_grad():
            ...
            if continuity:
                ...
                if self.audio:
                    self.audioencoder.eval()
                    audio = self.audioencoder(aud_feat.transpose(1, 2), frame_num=frame).unsqueeze(dim=-1).repeat(1, 1, 1, 2) # torch.Size([1, 256, 300//4, 2])
                    latents = self.generator.generate(id, shape=[audio.shape[2], 2], batch_size=B, aud_feat=audio) # torch.Size([1, 75, 2])
                else:
                ...
                body, _ = self.g_body.decode(b=body_latents.shape[0], w=body_latents.shape[1], latents=body_latents) # (1, 39, 300)
                hand, _ = self.g_hand.decode(b=hand_latents.shape[0], w=hand_latents.shape[1], latents=hand_latents) # (1, 90, 300)

            pred_poses = torch.cat([body, hand], dim=1).transpose(1,2).cpu().numpy()

        output = pred_poses # (1, 300, 129)
