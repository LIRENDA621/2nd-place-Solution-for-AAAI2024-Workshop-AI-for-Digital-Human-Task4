...
class MultiVidData():
    def __init__(self, 
        ...
        self.num_pre_frames=num_pre_frames # 88
        ...
        if load_mode=='pickle':
            ...
            f = open(os.path.join(self.data_root, self.split+config.Data.pklname), 'rb+')
        ...
        ######################origin load method
            ...
            for speaker_name in self.speakers:
                ...
                for vid in tqdm(videos, desc="Processing training data of {}......".format(speaker_name)):
                    ...
                    if smplx == 'pose':
                        seqs = [s for s in os.listdir(vid_pth) if (s.startswith('clip'))] # seq里面是一个pkl和wav文件
                    else:
                        try:
                    ...
                    for s in seqs:
                        ...
                        self.dataset[key]=dataset(  # self.dataset[key]的value是类SmplxDataset
                            data_root=seq_root,
                            speaker=speaker_name,
                            motion_fn=motion_fname,
                            audio_fn=audio_fname,
                            audio_sr=audio_sr, # 22000
                            fps=num_frames, # 88
                            feat_method=feat_method,
                            audio_feat_dim=aud_feat_dim,
                            train=(self.split=='train'),
                            load_all=True,
                            split_trans_zero=self.split_trans_zero, # false
                            limbscaling=self.limbscaling, # false
                            num_frames=self.num_frames, # 88
                            num_pre_frames=self.num_pre_frames, # 0
                            num_generate_length=self.num_generate_length, # 88
                            audio_feat_win_size=aud_feat_win_size,
                            context_info=context_info, # false
                            convert_to_6d=convert_to_6d, # false
                            expression=expression, # true
                            config=self.config,
                            am=am,
                            am_sr=am_sr, # 16000
                            whole_video=config.Data.whole_video # false / face=True
                        )
                        self.complete_data.append(self.dataset[key].complete_data) # 只有motion audio在self.dataset[key].audio_fea
                        haode = haode + 1
            ...
            # -----------------------
            f = open(os.path.join(self.data_root, self.split+config.Data.pklname), 'wb')
            pickle.dump(self.dataset, f)
            f.close()
        ######################origin load method

        self.complete_data=np.concatenate(self.complete_data, axis=0) # bs*clip, 265
    ...
    def get_dataset(self):
        ...
        for key in list(self.dataset.keys()):
            if self.dataset[key].complete_data.shape[0] < self.num_generate_length: # 如果当前clip的帧小于self.num_generate_length，就跳过
                continue
            self.dataset[key].num_generate_length = self.num_generate_length
            self.dataset[key].get_dataset(self.normalization, self.normalize_stats, self.split) # self.dataset[$key$].all_dataset self.dataset[key]=class Sim...
            self.all_dataset_list.append(self.dataset[key].all_dataset) # list，size=clip_num, 每个Clip间隔六帧分割
        
        if self.split_trans_zero:
            self.trans_dataset = data.ConcatDataset(self.trans_dataset_list)
            self.zero_dataset = data.ConcatDataset(self.zero_dataset_list)
        else: # len = self.cumulative_sizes[-1]
            self.all_dataset = data.ConcatDataset(self.all_dataset_list) # 每个Clip帧数减去num_generate,间隔六帧分割后，clip产生新clip数量的累加
