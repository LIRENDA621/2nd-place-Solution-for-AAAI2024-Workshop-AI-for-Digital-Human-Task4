...
with open('/nws/user/lirenda621/Code/TalkSHOW/data_utils/hand_component.json') as file_obj:
...
class SmplxDataset():
    ...
    def __init__(self,
        ...
        self.audio_sr = audio_sr # 22000
        self.fps = fps
        self.audio_feat_dim = audio_feat_dim
        self.audio_feat_win_size = audio_feat_win_size # none
        self.context_info = context_info  # for aud feat 未来也许可以加别的context
        ...
        if load_mode == 'pickle':
            ...
            self.annotations = glob(data_root + '/*pkl') # data root -> clip
            ...
            if self.load_all:
                self._load_them_all(am, am_sr, motion_fn) # self.complete_data(300, 265) self.audio_feat (300, 64)
    ...
    def _load_npz_all(self):
        ...
        for index in range(shape):
            ...
            full_body = np.concatenate(
                (jaw_pose, leye_pose, reye_pose, global_orient, body_pose, left_hand_pose, right_hand_pose)) # 3+3+3+3+63+12+12
    ...
    def get_dataset(self, normalization=False, normalize_stats=None, split='train'):
        ...
        class __Worker__(data.Dataset):
            ...
            def __getitem__(child, index):
                ...
                index_new = index + random.randrange(0, 5, 3) # random.randrange(0, 5, 3)只能生成0或3为什么?
                ...
                if child.split in ['val', 'pre', 'test'] or self.whole_video:
                    index = 0
                    seq_len = self.complete_data.shape[0] # self.complete_data.shape的self指class.sim...
                seq_data = []
                assert index + seq_len <= self.complete_data.shape[0]
                # print(seq_len)
                seq_data = self.complete_data[index:(index + seq_len), :] # (88,265)
        ...
        if split == 'train':
            index_list = list(
                range(0, min(self.complete_data.shape[0], self.audio_feat.shape[0]) - self.num_generate_length - self.num_pre_frames,
                      6))
        elif split in ['val', 'test']: # 推理的时候处理整个视频clip
            index_list = list([0])
        if self.whole_video:
            index_list = list([0])
        self.all_dataset = __Worker__(index_list, normalization, normalize_stats, split) # 这个是对于每一个clip的all_dataset
